...
func ServeDHCP(logger logr.Logger) error {
...
func handlePacket(logger logr.Logger) func(conn net.PacketConn, peer net.Addr, m *dhcpv4.DHCPv4) {
	return func(conn net.PacketConn, peer net.Addr, m *dhcpv4.DHCPv4) {
		...
		serverIP := net.ParseIP("172.24.0.3")

		resp, err := offerDHCP(m, serverIP, fwtype)
		if err != nil {
...
func offerDHCP(req *dhcpv4.DHCPv4, serverIP net.IP, fwtype Firmware) (*dhcpv4.DHCPv4, error) {
	...
	case FirmwareX86HTTP:
		...
		resp.UpdateOption(dhcpv4.OptBootFileName(fmt.Sprintf("http://%s/snp.ipxe", serverIP)))
	case FirmwareARMHTTP:
		// This is completely standard HTTP-boot: just load a file from HTTP.
		resp.UpdateOption(dhcpv4.OptBootFileName(fmt.Sprintf("http://%s/snp-arm64.ipxe", serverIP)))
