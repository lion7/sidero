...
func ServeDHCP() error {
	...
	for {
		...
		if err = isBootDHCP(pkt); err != nil {
			log.Printf("[DHCP] Ignoring packet from %s: %s", pkt.HardwareAddr, err)
			continue
		}
		...
		log.Printf("[DHCP] Offering to boot %s", pkt.HardwareAddr)
...
func offerDHCP(pkt *dhcp4.Packet, serverIP net.IP, fwtype Firmware) (*dhcp4.Packet, error) {
	...
	case FirmwareX86PC:
		...
		resp.BootFilename = fmt.Sprintf("%s/%d", mac, fwtype)
	...
	case FirmwareX86Ipxe:
		...
		resp.Options[43] = bs
		resp.BootFilename = fmt.Sprintf("tftp://%s/%s/%d", serverIP, mac, fwtype)
